...
{
  sem_wait(&(bank->checkForReportTransfer));
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&(bank->checkForReportTransfer));
...
{
  sem_wait(&(bank->checkForLastOne));

  bank->numberWorkers--;

  if (bank->numberWorkers > 0) {
    sem_post(&(bank->checkForLastOne));
    sem_wait(&(bank->checkForNextDay));
  } else if (bank->numberWorkers == 0) {
    Report *rpt = bank->report;

    assert(rpt); Y;

    bank->numberWorkers = bank->allNumberWorkers;
    sem_post(&(bank->checkForLastOne));

    if (rpt->numReports >= MAX_NUM_REPORTS) {
      for (unsigned int i = 0; i < (bank->numberWorkers - 1); i++) {
        sem_post(&(bank->checkForNextDay));
      }
